...
public class DefaultXmlArtifactDeclarationLoader implements XmlArtifactDeclarationLoader {
  ...
  private Map<String, ExtensionModel> extensionsByNamespace = new HashMap<>();

  public DefaultXmlArtifactDeclarationLoader(DslResolvingContext context) {
    this.context = context;
    this.resolvers = context.getExtensions().stream().collect(toMap(e -> e,
                                                                    e -> DslSyntaxResolver.getDefault(e, context)));
  }

  @Override
  public ArtifactDeclaration load(String configResource) {
    ...
    try {
      ...
      ConfigLine configLine = new XmlApplicationParser(new XmlApplicationServiceRegistry(new SpiServiceRegistry(), context))
    ...
    new ExtensionWalker() {
      ...
      protected void onConfiguration(ConfigurationModel model) {
        ...
        if (elementDsl.getElementName().equals(configLine.getIdentifier())) {
          ...
          configurationDeclarer.withRefName(configLine.getConfigAttributes().get(NAME_ATTRIBUTE_NAME).getValue());
          ...
          // TODO ORDER?
          List<ConfigLine> children = configLine.getChildren().stream()
            .filter(config -> {
          ...
          declareParameterizedComponent(model, elementDsl, configurationDeclarer, attributes, children);
      ...
      protected void onParameter(ParameterizedModel owner, ParameterGroupModel groupModel, ParameterModel model) {
        ...
        if (elementDsl.supportsTopLevelDeclaration() && elementDsl.getElementName().equals(configLine.getIdentifier())) {
          ...
          topLevelParameter.withRefName(configLine.getConfigAttributes().get(NAME_ATTRIBUTE_NAME).getValue());
  ...
  private void declareFlow(ConfigLine configLine, ArtifactDeclarer artifactDeclarer) {
    Map<String, SimpleConfigAttribute> attributes = configLine.getConfigAttributes();

    final FlowElementDeclarer flow = ElementDeclarer.newFlow(attributes.get(NAME_ATTRIBUTE_NAME).getValue());
